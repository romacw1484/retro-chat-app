...
app = Flask(__name__)
CORS(app)  # Now that app is defined, we can call CORS(app)
...
def save_message():
    ...
    print(f"Saving message to {recipient_username}: {content}")
    sender_id = session.get('user_id')
    if not sender_id:
        return jsonify({'status': 'Not logged in'})
...
def handle_connect():
    ...
    if user_id:
        ...
        print("Session not found for WebSocket connection!")
...
def handle_message(data):
    ...
    if receiver:
        new_message = Message(sender_id=sender_id, receiver_id=receiver.id, content=message_content)
        db.session.add(new_message)
        db.session.commit()
        
        print(f"Message saved: {message_content}")
        
        emit('message', {'username': sender_username, 'message': message_content}, room=receiver.id)
        emit('message', {'username': sender_username, 'message': message_content}, room=sender_id)
    else:
        print(f"Recipient {recipient_username} not found")

if __name__ == '__main__':
    # We are using migrations, so we don't call db.create_all() here.
